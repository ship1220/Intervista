...
from ai_service import generate_content, extract_json, evaluate_answers
from prompts import (
...
async def api_interview_evaluate(request: Request, db: Session = Depends(get_db)):
    ...
    if "questions_answers" in body:
        # New format with speech analysis
        ...
        role = session.get("role", "Software Developer")  # Default if not set
        
        # Analyze speech for each answer
        voice_analysis = []
        for qa in questions_answers:
            metrics = analyze_speech_metrics(qa["answer"], qa["duration"])
        ...
        from ai_service import evaluate_answers
        content_evaluation = await evaluate_answers(role, questions_answers)
        
        # Aggregate results
        overall_score = 0
        total_questions = len(questions_answers)
        if total_questions > 0:
            # Simple aggregation: average clarity scores, plus content score
            avg_clarity = sum(m["metrics"]["sentence_clarity_score"] for m in voice_analysis) / total_questions
            # Assume content has some score, but since not provided, use arbitrary
            content_score = 7.0  # Placeholder
            overall_score = round((avg_clarity + content_score) / 2, 1)
        
        recruiter_verdict = "Strong candidate" if overall_score >= 7 else "Needs improvement" if overall_score >= 5 else "Not recommended"
        
        report = {
            "overall_score": overall_score,
            "recruiter_verdict": recruiter_verdict,
            "voice_analysis": voice_analysis,
            "content_analysis": content_evaluation,
            "detailed_answer_report": content_evaluation.get("feedback_per_question", [])
        }
...
async def api_course_stream(request: Request, role: str, level: str, db: Session = Depends(get_db)):
    ...
    async def event_generator():
        ...
        outline_raw = await generate_content(course_outline_prompt(role, level))
        outline = None
        try:
